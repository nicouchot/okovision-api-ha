...
$(document).ready(function() {
	...
	$('#bt_regen_token').click(function() {
		...
		if (!confirm('Générer un nouveau token invalidera l\'ancien. Continuer ?')) {
			return;
		}

		$.api('POST', 'admin.regenerateToken', {}, false).done(function(json) {
			if (json.response) {
				$('#api_token_display').val(json.token);
				$.growlValidate('Nouveau token généré. Mettez à jour votre intégration Home Assistant.');
			} else {
				$.growlWarning('Erreur lors de la génération du token.');
			}
		});
	});
	...
	$('#bt_save_infoge').click(function() {
		...
		var tab = {
			...
			pci_pellet : $('#param_pci_pellet').val(),
			rendement : $('#param_rendement').val(),
